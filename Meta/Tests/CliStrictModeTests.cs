...
public sealed partial class CliStrictModeTests
{
    ...
    [Fact]
    public async Task BulkInsert_RejectsHeaderWithoutDataRows()
    {
        var workspaceRoot = CreateTempWorkspaceFromSamples();
        try
        {
    ...
    [Fact]
    public async Task BulkInsert_RejectsEmptyInputWithoutChangingWorkspace()
    {
        var workspaceRoot = CreateTempWorkspaceFromSamples();
        try
        {
            var workspaceService = new WorkspaceService();
            var before = await workspaceService.LoadAsync(
                workspaceRoot,
                searchUpward: false);
            var beforeCount = before.Instance
            ...
            Assert.Contains(
                ...
                result.CombinedOutput,
                StringComparison.OrdinalIgnoreCase);
            var after = await workspaceService.LoadAsync(
                workspaceRoot,
                searchUpward: false);
            Assert.Equal(
                beforeCount,
                after.Instance.GetOrCreateEntityRecords("Cube").Count);
        }
        finally
        {
            DeleteDirectorySafe(workspaceRoot);
