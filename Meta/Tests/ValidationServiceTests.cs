...
public sealed class ValidationServiceTests
{
    ...
    [Fact]
    public void Validate_UnknownInstanceMembers_AreErrors()
    {
        var workspace = BuildWorkspace(
            modelName: "MetadataModel",
            entityName: "Cube",
            propertyName: "Name");
        workspace.Instance.GetOrCreateEntityRecords("Unknown").Add(
            new GenericRecord
            {
                Id = "unknown-1",
            });
        var cube = new GenericRecord
        {
            Id = "cube-1",
            Values =
            {
            ...
            RelationshipIds =
            {
                ["UnknownRelationshipId"] = "target-1",
            },
        };
        workspace.Instance.GetOrCreateEntityRecords("Cube").Add(cube);

        var diagnostics = new ValidationService().Validate(workspace);

        Assert.Contains(
            diagnostics.Issues,
            issue => issue.Code == "instance.entity.unknown");
        Assert.Contains(
            diagnostics.Issues,
            issue => issue.Code == "instance.property.unknown");
        Assert.Contains(
            diagnostics.Issues,
            issue => issue.Code == "instance.relationship.unknown");
        Assert.True(diagnostics.HasErrors);
    }

    [Fact]
    public void Validate_IdentityWithSurroundingWhitespace_IsError()
    {
        var workspace = BuildWorkspace(
            modelName: "MetadataModel",
            entityName: "Cube",
            propertyName: "Name");
        workspace.Instance.GetOrCreateEntityRecords("Cube").Add(
            new GenericRecord
            {
                Id = " cube-1 ",
                Values =
        ...
        Assert.Contains(
            diagnostics.Issues,
            issue => issue.Code == "instance.id.invalid");
        Assert.True(diagnostics.HasErrors);
    }
