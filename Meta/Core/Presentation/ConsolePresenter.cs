...
{
    public void WriteOk(string? summary = null, params (string Key, string Value)[] details)
    {
        _ = summary;
        _ = details;
    ...
    public void WriteFailure(string message, IEnumerable<string>? details = null)
    {
        ...
        var intent = NormalizeCannotContinueIntent(normalizedMessage);
        if (string.Equals(intent, "Cannot continue.", StringComparison.Ordinal) &&
            !string.IsNullOrWhiteSpace(normalizedMessage))
        {
            nonNextDetails.Insert(0, normalizedMessage);
        }

        WriteCannotContinue(intent, nonNextDetails, next);
    }

    public void WriteCannotContinue(
        string intent,
        IEnumerable<string>? details = null,
        string? next = null)
    {
        var renderedIntent = NormalizeCannotContinueIntent(intent);
        Console.WriteLine(renderedIntent);
        ...
        var renderedNext = Normalize(next);
        if (!string.IsNullOrWhiteSpace(renderedNext))
        {
            Console.WriteLine();
            Console.WriteLine(renderedNext.StartsWith("Next:", StringComparison.OrdinalIgnoreCase)
                ? renderedNext
                : $"Next: {renderedNext}");
        }
    }
    ...
    private static string NormalizeCannotContinueIntent(string? value)
    {
        var normalized = NormalizeErrorMessage(value);
        if (string.IsNullOrWhiteSpace(normalized))
        {
            return "Cannot continue.";
        }

        if (normalized.StartsWith("Cannot ", StringComparison.OrdinalIgnoreCase))
        {
            return EnsureSentence(normalized);
        }

        if (normalized.EndsWith("failed.", StringComparison.OrdinalIgnoreCase) ||
            normalized.EndsWith("failed", StringComparison.OrdinalIgnoreCase))
        {
            return EnsureSentence(normalized);
        }
    ...
    private static string NormalizeErrorMessage(string? value)
    {
        ...
        if (normalized.StartsWith(ErrorPrefix, StringComparison.OrdinalIgnoreCase))
        {
            normalized = normalized[ErrorPrefix.Length..].Trim();
        }
