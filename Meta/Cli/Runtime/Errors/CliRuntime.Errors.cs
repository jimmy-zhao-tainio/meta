internal sealed partial class CliRuntime
{
    ...
    {
        var message = NormalizeErrorMessage(document.Message);
        var intent = BuildFailureIntent(message);
        ...
        presenter.WriteInfo(intent);

        void EnsureBodyStarted()
        {
            if (wroteBody)
            {
                return;
        ...
        {
            EnsureBodyStarted();
            if (wroteBody && !string.Equals(intent, "Cannot continue.", StringComparison.Ordinal))
            {
                presenter.WriteInfo(string.Empty);
            }
        ...
        if (hasBody)
        {
            ...
            if (hasUsage || hasDetails ||
                (string.Equals(intent, "Cannot continue.", StringComparison.Ordinal) &&
                 !string.IsNullOrWhiteSpace(message)))
            {
                presenter.WriteInfo(string.Empty);
            }
    ...
    string BuildFailureIntent(string message)
    {
        ...
        return normalized.StartsWith("Cannot ", StringComparison.OrdinalIgnoreCase)
