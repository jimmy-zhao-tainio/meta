...
| 23. Generation and CSV export inputs | `meta generate sql/csharp/ssdt` and `meta export csv` | SQL, C#, and SSDT generation consume `InMemoryWorkspace`; CSV export consumes `IMetaWorkspaceSource`; tooling generation receives its optional source path explicitly | Generation `Workspace` overloads and CSV export object-graph traversal | Complete |
...
Graph diagnostics deliberately remain on the raw loaded model. They report malformed declarations, including duplicate entities and missing relationship targets. `IMetaWorkspaceSource` currently assumes an addressable, unambiguous model and would collapse or reject precisely the malformed structure those commands must diagnose. Moving graph analysis requires a separate diagnostic-source contract; forcing it through the normal semantic source would be a behavior regression.
...
## Remaining Legacy Engines

- Artifact-directory emission still sits in `GenerationService`.
...
Graph analysis is an intentional malformed-model diagnostic boundary, not ordinary migration debt, until a diagnostic source can preserve duplicate and unresolved declarations.

Delete entries from this list only when their implementation and all references are gone.
