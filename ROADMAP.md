...
- Keep `MetaDataTypeConversion` responsible for conversion rules, not source discovery.
...
Long term:
- `Field` should carry enough structured source-type identity to resolve into `MetaDataType` cleanly.
...
This is the shared type vocabulary that both `MetaSchema` and `MetaDataTypeConversion` consume.
...
This should consume `MetaDataType`, not duplicate its core concepts.
...
## Phase 2: Define `MetaDataType`

Goal:
- introduce the sanctioned shared type model

Deliverables:
- new sanctioned `MetaDataType` model
...
4. Add `meta-schema` or `meta` seed path for `MetaDataType`.
...
## Phase 3: Make `MetaSchema` Target `MetaDataType`

Goal:
- stop minting ad hoc extracted `FieldType` identities

Deliverables:
- `MetaSchema` updated so field-level type identity is compatible with `MetaDataType`
...
Exit condition:
- extracted fields identify types in the shared `MetaDataType` universe.
...
## Phase 6: Rebuild `MetaDataTypeConversion` on `MetaDataType`

Goal:
- make conversion rules consume the shared type model instead of parallel duplicated concepts

Deliverables:
- `MetaDataTypeConversion` sanctioned model aligned to `MetaDataType`

Tasks:
1. Review current `TypeConversionCatalog` concepts and rename/reframe as needed.
2. Remove duplicate type identity concepts if `MetaDataType` now owns them.
...
2. Introduce `MetaDataType`
3. Repoint `MetaSchema` field type identity into `MetaDataType`
4. Introduce sanctioned `MetaWeave`
5. Implement `meta-weave check`
6. Rework `MetaDataTypeConversion` to consume `MetaDataType`
7. Only then consider merged/materialized views

## Risks

1. Monolith drift
- trying to solve `MetaSchema`, `MetaDataType`, `MetaDataTypeConversion`, and `MetaWeave` in one schema will collapse concerns and slow iteration

2. Hidden runtime conventions
- if `TypeId` resolution is hardcoded instead of woven, the model split becomes fake

3. Type duplication
- if `MetaSchema` keeps its own `FieldType` world while `MetaDataTypeConversion` keeps another, the architecture will remain muddy
...
2. Start `MetaDataType` as a sanctioned model with the smallest viable type vocabulary.
