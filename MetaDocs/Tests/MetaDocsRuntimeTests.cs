...
public sealed class MetaDocsRuntimeTests
{
    ...
    public void Cli_HelpIsDerivedFromAuthoredMetaCliWorkspace()
    {
        ...
        Assert.Contains("cli-matrix", appHelp.Output);
        ...
        Assert.Contains("(--output-xml <workspace> | --output-csharp <workspace> | --output-sql <workspace>)", mergeHelp.Output);

        var mergeWithoutOutput = RunCli("merge --include missing-workspace");
        Assert.Equal(2, mergeWithoutOutput.ExitCode);
        Assert.Contains("Parameter group 'output' requires one of: output-xml, output-csharp, output-sql.", mergeWithoutOutput.Output);

        var mergeWithConflictingOutputs = RunCli("merge --include missing-workspace --output-xml out-xml --output-csharp out-csharp");
        Assert.Equal(2, mergeWithConflictingOutputs.ExitCode);
        Assert.Contains("Parameter group 'output' accepts only one member.", mergeWithConflictingOutputs.Output);
        ...
        Assert.Contains("--subject-type <value>", searchHelp.Output);
        Assert.DoesNotContain("--kind", searchHelp.Output);

        var authorPageHelp = RunCli("help author-page");
        Assert.Equal(0, authorPageHelp.ExitCode);
        Assert.Contains("--subject-type <value>", authorPageHelp.Output);
        Assert.Contains("Defaults to Guide", authorPageHelp.Output);
        Assert.DoesNotContain("--kind", authorPageHelp.Output);
    ...
    public void CliMatrix_GeneratesClassifiedCommandsAndInvocationConformance()
    {
        ...
        var commandIds = suite.DocumentationSubjectList
            .Where(IsActive)
            .Where(subject => IsSubjectType(subject, "CliCommand"))
            .Select(static subject => subject.Id)
            .ToHashSet(StringComparer.OrdinalIgnoreCase);
        var expectedParameterCount = suite.DocumentationSubjectList.Count(subject =>
            IsActive(subject) &&
            subject.ParentSubject is not null &&
            commandIds.Contains(subject.ParentSubject.Id) &&
            (IsSubjectType(subject, "CliOption") || IsSubjectType(subject, "CliArgument")));
        var expectedGroupCount = suite.DocumentationSubjectList.Count(subject =>
            IsActive(subject) &&
            subject.ParentSubject is not null &&
            commandIds.Contains(subject.ParentSubject.Id) &&
            IsSubjectType(subject, "CliParameterGroup"));
        Assert.Equal(expectedParameterCount, matrix.ParameterCount);
        ...
        try
        {
            ...
            Assert.Equal(0, cohortResult.ExitCode);
            Assert.Contains($"Wrote CLI matrix (cohorts, {matrix.DecisionCohorts.Count} row(s))", cohortResult.Output);
    ...
    public void Cli_BrowseSearchAndUpdateDescriptionDefaultWorkspaceToCurrentDirectory()
    {
        ...
        try
        {
            ...
            Assert.Equal(
                "Overview",
                Assert.Single(model.DocumentationViewNodeList, row => row.DocumentationSubject?.Id == reference.MetaOverview.Id).Title);
            ...
            Assert.Contains("meta-docs search meta", metaBrowse.Output);
    ...
    public void SuiteMerge_PreservesAuthoredReferenceRootAndGeneratedSubjectParents()
    {
        ...
        Assert.Equal(
            "public:meta:cli",
            Assert.Single(suite.DocumentationSubjectList, row =>
                row.DisplayName == "meta" && IsSubjectType(row, "CliApplication")).ParentSubject?.Id);
    ...
    public void RenderSite_UsesSinglePanelRoutesAndModeledPublicHierarchy()
    {
        ...
        Assert.Contains(">Overview</a>", html, StringComparison.Ordinal);
        Assert.Contains("<h2>meta</h2>", html, StringComparison.Ordinal);
        Assert.Contains("<h2>meta-bi</h2>", html, StringComparison.Ordinal);
        ...
        Assert.Contains("<code>inline code</code>", html, StringComparison.Ordinal);

        var browseText = new MetaDocsBrowseService().Browse(model, "meta/overview").Text;
        Assert.Contains("Use strong emphasis and inline code in authored documentation.", browseText, StringComparison.Ordinal);
        Assert.DoesNotContain("**strong emphasis**", browseText, StringComparison.Ordinal);
        Assert.DoesNotContain("`inline code`", browseText, StringComparison.Ordinal);
